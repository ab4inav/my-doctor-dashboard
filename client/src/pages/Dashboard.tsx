...
export default function Dashboard() {
  ...
  const { data: patients = [] } = useQuery({
    ...
    queryFn: () =>
      doctor ? getPatientsByDoctor(doctor.id) : Promise.resolve([]),
    enabled: !!doctor,
  });

  const { data: prescriptionCount = 0 } = useQuery({
    queryKey: ["prescriptionCount", doctor?.id],
    queryFn: () =>
      doctor ? getPrescriptionCountByDoctor(doctor.id) : Promise.resolve(0),
    enabled: !!doctor,
  });
  ...
  return (
    <div className="p-6">
        ...
        <Card className="bg-white shadow-sm border border-medical-gray-200">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-medical-gray-600 text-sm font-medium">
                  Total Patients
                </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-white shadow-sm border border-medical-gray-200">
          <CardContent className="p-6">
            <h3 className="text-lg font-semibold text-medical-gray-800 mb-4">
              Recent Patients
            </h3>
